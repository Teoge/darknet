...
void fill_truth_ps(char *path, int num_boxes, float *truth, int flip)
{
	char labelpath[4096];
	find_replace(path, "images", "labels", labelpath);
	find_replace(labelpath, "JPEGImages", "labels", labelpath);
	...
	box_label *boxes = calloc(1, sizeof(box_label));
	while (fscanf(file, "%d %f %f %f %f", &id, &x, &y, &angle, &dist) == 5 && count <= num_boxes) {
		boxes = realloc(boxes, (count + 1) * sizeof(box_label));
		boxes[count].x = x;
		boxes[count].y = y;
		boxes[count].w = angle;
	...
	int i;
	for (i = 0; i < count; ++i) {
		truth[i * 5 + 0] = boxes[i].x;
		truth[i * 5 + 1] = boxes[i].y;
		truth[i * 5 + 2] = boxes[i].w;
		truth[i * 5 + 3] = boxes[i].h;
		truth[i * 5 + 4] = boxes[i].id;
	}
	free(boxes);
}
...
data load_data_ps_region(int n, char **paths, int m, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure)
{
	char **random_paths = get_random_paths(paths, n, m);
	int i;
	data d = { 0 };
	...
	for (i = 0; i < n; ++i) {
		...
		fill_truth_ps(random_paths[i], boxes, d.y.vals[i], flip);
	}
	free(random_paths);
	return d;
}
...
void *load_thread(void *ptr)
{
    ...
    } else if (a.type == PS_DATA) {
		*a.d = load_data_ps_region(a.n, a.paths, a.m, a.w, a.h, a.num_boxes, a.classes, a.jitter, a.hue, a.saturation, a.exposure);
